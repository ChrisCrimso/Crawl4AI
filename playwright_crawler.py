...
This script extracts RAW TEXT ONLY from problematic URLs in the 404_issues directory,
removing all HTML and formatting to provide clean data for knowledge base systems.
"""
...
def extract_text_from_html(html_content):
    """
    Extract clean raw text from HTML content using BeautifulSoup.
    Removes all HTML tags, scripts, styles, and excess whitespace.
    """
    try:
        soup = BeautifulSoup(html_content, 'html.parser')
        
        # Remove all script, style, meta tags, etc.
        for element in soup(['script', 'style', 'head', 'title', 'meta', 'link', 'noscript', 
                            'header', 'footer', 'nav', 'iframe', 'svg']):
            element.decompose()
        ...
        text = re.sub(r'cookie[s]?\s+policy|privacy\s+policy|terms\s+of\s+service|all\s+rights\s+reserved', '', 
                      text, flags=re.IGNORECASE)
        
        # Remove social media text
        text = re.sub(r'follow us on|share on|connect with us on|follow on', '', text, flags=re.IGNORECASE)
...
def crawl_with_playwright(url, wait_time=5, wait_for_network_idle=True, timeout=30000):
    ...
    try:
        with sync_playwright() as p:
            ...
            # Extract raw text content - THIS IS THE KEY STEP
            ...
            return {
                "success": True,
                "content": text_content,  # Store ONLY the raw text
                "title": title,
...
def save_crawled_content(metadata, crawl_result, output_dir=OUTPUT_DIR):
    ...
    md_content += f"content_type: raw_text\n"  # Explicitly mark as raw text
    if not crawl_result.get('success', False):
        md_content += f"error: {crawl_result.get('error', 'Unknown error')}\n"
    md_content += "---\n\n"
    
    # Add the content - RAW TEXT ONLY
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Crawl problematic URLs using Playwright and extract raw text.')
    ...
    for metadata in tqdm(urls):
        ...
        result_entry = {
            ...
            "content_type": "raw_text"  # Mark explicitly as raw text
        }
