...
def clean_text_content(text_content):
    """Clean text content for comparison by removing extra whitespace."""
    try:
...
def compare_file_content(original_file, recrawled_file):
    ...
    # Clean text content
    original_text = clean_text_content(original_content)
    recrawled_text = clean_text_content(recrawled_content)
    ...
    original_words = len([w for w in original_text.split() if len(w) > 3])
    recrawled_words = len([w for w in recrawled_text.split() if len(w) > 3])
    ...
    return {
        ...
        "recrawled_words": recrawled_words,
...
def generate_comparison_report(comparisons):
    ...
    total_original_words = sum(c["original_words"] for c in comparisons if c["crawl_success"])
    total_recrawled_words = sum(c["recrawled_words"] for c in comparisons if c["crawl_success"])
    word_increase = total_recrawled_words - total_original_words
    word_improvement_percent = (word_increase / total_original_words * 100) if total_original_words > 0 else float('inf')
    ...
    <body>
        ...
        <div class="summary">
            ...
            <p>Average content length improvement: {avg_improvement:.2f}%</p>
            <p>Total meaningful words in original files: {total_original_words}</p>
            <p>Total meaningful words in recrawled files: {total_recrawled_words}</p>
            <p>Word count improvement: +{word_increase} words ({word_improvement_percent:.2f}%)</p>
        </div>
        ...
        <div class="comparison success">
            ...
            <div class="stats">
                <div class="stat">
                    ...
                    <strong>Original Meaningful Words:</strong> {comparison["original_words"]}<br>
                    <strong>Recrawled Meaningful Words:</strong> {comparison["recrawled_words"]}<br>
                    <span class="improvement {improvement_class}">
                        Length Improvement: {comparison["improvement_percentage"]:.2f}%<br>
                        Word Improvement: {(comparison["recrawled_words"] - comparison["original_words"])} words
                    </span>
                </div>
                <div class="stat">
    ...
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump({
            "summary": {
                ...
                "total_recrawled_words": total_recrawled_words,
                "word_improvement": word_increase,
                "word_improvement_percent": word_improvement_percent
            },
